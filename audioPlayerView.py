    ...
    def load_Playlist_From_File(self):
        try:
            with open("MP3MusicList.txt", "r", encoding='UTF-8') as playlist_File:
                ...
                for element in list:
                    if not element.isspace(): # kiểm tra phần tử không phải là một phần tử khoảng trắng hoàn toàn
                        self.lines.append(element) # nếu không phải thì được add vào
                for filePath in self.lines: #kiểm tra từng đường dẫn trong list
                    index = self.list_song.size()
   ...
   #show current time of song and show on the screen
    def show_details(self, play_song):
        ...
        if file_data[1] == ".mp3": # nếu thư mục là mp3
            ...
            a = mixer.Sound(play_song) # dùng pygame mxier.Sound() để lấy độ dài của bài hát
